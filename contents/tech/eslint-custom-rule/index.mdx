...
여러분은 팀내에서 컨벤션을 유지하기 위해 ESLint를 사용하고 계신가요? 요즘은 개인 프로젝트에서도 ESLint를 도입하여 일관된 컨벤션을 유지하는 만큼 대부분의 프론트엔드 개발자 분들이 팀내에서 eslint를 사용하고 있을것입니다.

왠만한 규칙들은 이미 ESLint 생태계에 존재하기 때문에 보통 팀내에서 결정된 컨벤션에 맞는 규칙을 찾기만 하면 되는 경우가 많습니다. 하지만 도메인이 반영된 코드를 작성하거나 팀내에서 만들어진 라이브러리를 사용하는 경우를 위한 규칙은 일반적이지 않기 때문에 존재하지 않을 가능성이 높습니다.

이때 ESLint에서 제공하는 API를 이용하면 우리 팀에만 존재하는 컨벤션을 ESLint 규칙으로 만들어 팀내 컨벤션이 보다 잘 유지되도록 할 수 있습니다.

이번 아티클에서는 ESLint의 동작원리와 API에 대해서 살펴보고 이를 바탕으로 간단한 ESLint 규칙을 만들어 보겠습니다.


## ESlint의 오류 탐색 방식
ESLint는 어떤 방식으로 코드에서 오류를 찾아내고 이를 수정해주는것일까요? == 대신 === 연산자를 사용하도록 강제하는 eqeqeq라는 ESLint 규칙을 예시로 하여 알아봅시다.

가장 간단하게 생각해볼수 있는건 Ide에서 제공하는 리팩토링처럼 코드 내에서 ==를 찾아서 경고한뒤 --fix 옵션을 사용하면 모두 ===로 바꿔주는것입니다. 큰 문제가 없어보이지만, 연산자가 아닌 문자열 내의 ==도 ===로 변경해주므로 아래와 같은 상황이 발생할 수 있습니다.
```
...
따라서 ESLint는 소스코드에서 키워드를 검색하는 방식으로 오류를 탐색하는것이 아니라 소스코드를 AST라는 의미있는 정보로 변환한뒤 이를 기반으로 소스코드 내에서 오류를 탐색합니다.

AST를 이해한다면 ESLint에서 규칙을 작성하는 방법의 80퍼센트 이상을 이해한것이나 다름없기 때문에 아래에서 많은 지면을 할애하여 AST에 대해서 살펴보겠습니다.
...
AST파서를 통해 소스코드로부터 AST가 생성되기까지는 두번의 변환이 존재합니다. 정의된 규칙을 사용하여 문자를 읽어 이를 토큰으로 결합하는 어휘 분석 단계와 이 토큰들을 가져와 AST로 변환하는 파서 단계를 거치면 AST가 생성됩니다.

### AST 상세 분석
AST는 노드라는 기본단위가 존재합니다. 노드란 문법적으로 의미있는 단위로 함수, 객체, 변수등이 여기에 해당합니다. 일반적으로 노드에는 타입(type)과 위치 정보(loc, start, end, range)가 공통적으로 존재하며 타입별로 필요한 정보가 추가적으로 존재합니다.

AST 분석을 위해 소스코드가 필요하므로 상단에 존재하는 이미지 내의 코드인 `const name = "foo"`를 사용하여 코드를 작성해보겠습니다.
...
```
AST의 최상위 노드의 타입은 Program 입니다. 우리가 일반적으로 사용하는 개념인 전역객체와 동일합니다. 이 타입에는 추가적인 프로퍼티가 세가지 존재합니다.
...
일반적으로 소스코드의 정보에 접근하기 위해 body를 사용합니다. 위 예제코드의 경우 소스코드가 한줄이기 때문에 body배열 길이도 1입니다. 따라서 body의 첫번째 요소 노드에 대해서 살펴봅시다. 
...
- init: 초기값 노드가 담깁니다. 여기서는 "foo"가 여기에 해당합니다.
...
`const name = "foo"` 소스코드의 AST를 분석해보았습니다. 기억해야할것은 최상위 노드의 타입이 Program인것과 Program 노드의 내부에 존재하는 노드들은 여러 타입들이 존재하며 노드는 또다른 노드를 프로퍼티에 포함하고 있을수 있습니다. 

또한 노드의 경우 앞서 살펴본 VariableDeclaration, VariableDeclarator, Literal, Identifier 외에도 화살표 함수를 의미하는 ArrowFunctionExpression, If문을 의미하는 IfStatement 등 수많은 타입이 존재합니다. 타입마다 추가적으로 존재하는 프로퍼티가 천차만별이고, 타입의 종류가 엄청 많기 때문에 모든 타입을 다 외울 필요는 없고 필요할때마다 [소스코드를 AST로 변환](https://astexplorer.net/)시켜 타입을 살펴보는것이 좋습니다.
...
AST에 대한 이해가 어느정도 쌓였다면 이제 추가적으로 알아야할 내용은 규칙 작성시 사용해야할 두가지 API, rule과 RuleTester 가 어떤 방식으로 동작하는지 살펴보는것입니다.

### rule
eslint에서 규칙을 만들때 규칙은 하나의 객체에 작성하게 되며 해당 객체에는 크게 객체인 meta와 함수인 create라는 프로퍼티를 가지고 있습니다.
...
#### meta 프로퍼티
meta에는 주로 rule에 대한 메타 정보가 담기게 됩니다. 객체이므로 프로퍼티에 해당 정보가 들어가게 되며 가능한 프로퍼티 별로 하나씩 알아보겠습니다.

- type(string): 규칙의 성격을 명시합니다. 아래와 같이 세가지가 가능합니다.
	- problem: 코드의 에러발생을 방지하는 규칙
  - suggestion: 코드를 더 좋게 변경하지만 에러발생을 막는것은 아닌 규칙
  - layoud: 공백, 세미콜론, 쉼표등 코드 모양과 관련된 규칙
...
일반적으로 type 정도만 작성하고 수정이 필요한경우 fixable, 옵션을 사용한다면 schema 정도를 사용하게 되며 나머지 프로퍼티는 크게 많이 사용하지는 않습니다. 중요한건 create 함수를 작성하는것이므로 본격적으로 해당 프로퍼티를 살펴보겠습니다.

#### create 프로퍼티
create 프로퍼티는 값으로 함수를 넘겨야하며, 해당 함수는 context라는 객체가 인수로 담겨 실행됩니다. 함수의 리턴값으로는 객체를 넘겨야 하며 이 객체 내부에는 순회할 AST 타입정보를 담게 됩니다.
...
options 프로퍼티를 사용하면 사용자가 eslint config 설정에 포함한 옵션값을 받을수 있습니다. 예를들면 아래와 같습니다.
```javascript
...
규칙을 작성하였으면 테스트도 작성할수 있어야합니다. 물론 규칙을 직접 코드에 적용해볼수도 있지만, 테스트 케이스를 적절하게 관리하고 커버리지를 관리하려면 테스트 코드는 필수입니다.

이러한 테스트 코드 작성을 손쉽게 해주는 도구가 ESLint에서 제공하는 RuleTester입니다. 이 도구르 이용하면 규칙을 보다 손쉽게 적용이 가능합니다.

먼저 RuleTester는 클래스로 사용을 위해 인스턴스화가 필요합니다. 이때 초기값으로 parser와 parserOptions를 넣을수 있습니다. parser의 경우 AST로 변환할 도구를 선택하는것입니다. 만약 넣지 않을경우 기본적으로 제공되는 파서를 선택하지만 typescript규칙을 작성할경우 @typescript-eslint/parser 와 같은 파서를 사용해 테스트 하는것이 필요합니다. 
...
> 앞서 살펴본 모든 내용들은 [ESLint의 커스텀 규칙 만들기 페이지](https://eslint.org/docs/latest/extend/custom-rules)에 매우 자세하게 설명이 되어있습니다. 더 자세한 설명이 필요하시면 위 문서를 참고해보세요

## Custom 만들어보기
이제 간단한 플러그인을 처음부터 끝까지 만들어보면서 이제까지 살펴보았던 내용을 모두 적용해보겠습니다.

만들고자 하는 플러그인은 @common 절대 경로를 가진 default import 에서 import 명은 반드시 불러온 파일 이름과 동일해야한다는 규칙이며, 이때 잘못 import하고 있는경우 수정해주는 기능을 포함하고 있습니다. 아래 예제 코드를 살펴보면 이해하기 쉽습니다. 
...
```
> 예제로 만드는 코드는 typescript코드를 고려하지 않기 때문에 typescript 파일에서 정상적으로 적용되지 않을수 있습니다. 이유는 typescript parser가 eslint의 기본 파서와 동작에 있어 차이가 존재하기 때문입니다. 기본적인 이해를 위해서 이러한 부분은 고려하지 않은점 양해 부탁드립니다.

### 프로젝트 세팅하기
먼저 플러그인을 위해 새로운 폴더를 생성합니다. 위치는 기존에 진행중인 프로젝트 내부도 좋고, 완전 새로운 폴더로 생성해도 좋습니다. 이후 초기화 한뒤 필요한 종속성을 설치합니다.

- eslint: custom-plugin 작성을 위한 여러 도구가 포함된 패키지
...
```
ImportDeclaration 타입의 노드에는 추가로 존재하는 프로퍼티가 specifiers, source 두가지입니다. 이를 좀더 살펴보면 specifiers의 경우 import 시 가져오는 모듈의 이름이고, source의 경우 모듈의 경로임을 알수 있습니다. 따라서 이 두프로퍼티를 이용하면 손쉽게 오류 검출 및 수정이 가능해 보입니다.

먼저 source에서 경로를 찾아봅시다. source의 경로는 source.value를 통해 쉽게 찾아낼수 있습니다. 여기에 "/"로 split하여 마지막 요소를 얻으면 import 이름으로 가져가야 하는 경우가 나오게 됩니다.

다음은 specifiers에서 import 모듈 이름을 찾아봅시다. specifiers은 배열로 여러개가 존재할수 있음을 알수 있습니다. 추측컨데 default가 아니라 객체형태로 import하는 경우를 대비한 케이스로 보입니다. 이를 증명하려면 객체형태의 import문을 파싱해보면됩니다. 

```javascript
import { A, B } from "@common/utils"
...
직접 만들어보면 default Import가 아닌경우 ImportSpecifier 라는 타입의 노드가 생성되며 또한 갯수에 맞춰 두개임을 알수 있습니다. 따라서 specifiers에서 ImportDefaultSpecifier타입의 노드를 찾아 local 프로퍼티에 존재하는 name을 찾아 앞서 찾은 규칙이름과 동일한지 비교하면 됩니다. 이 모든 내용을 코드로 담으면 다음과 같습니다. 주석에 자세히 설명해 두었으니 참고해보시면 좋겠습니다.
...
다만 모든 Identifier에서 name을 찾아 변경하면 문제가 생길수 있기에 예외조건이 필요합니다. 첫번째는 import문에서 검사를 굳이 할필요가 없기 때문에 그냥 패스하는것으고 두번째는 객체의 프로퍼티 `test.utilites`와 같은 형태도 utils로 변경하면 안되기에, 이에대한 예외처리가 필요합니다.

첫번째 예외처리는 import문 작성때처럼 해당 타입을 무시하면 되고, 두번째의 경우 eslint parser에서 부모의 노드를 parents라는 프로퍼티로 제공하므로 부모 노드의 propery 타입이 자기자신인경우, 즉 자신이 프로퍼티인경우 무시하는 로직을 추가하면 됩니다. 
...
이제 만들어진 플러그인을 직접 사용해 봅시다. 루트에 이 룰을 적용하기 위한 파일을 작성합니다.
```shell
touch eslint-plugin-bandi.js
```
파일에는 우리가 작성한 룰을 import하여 포함시켜 줍니다.
...
그리고 eslint.config.js 파일을 구성합니다.
```shell
touch eslint.config.js
```

eslint.config.js 파일에는 우리가 작성한 규칙이 포함되어야 합니다.
...
이제 `sample.js`파일을 만들고 해당 파일내부에 잘못된 규칙이 적용된 코드를 적은뒤 `npx eslint sample.js` 와 같이 eslint를 돌리면 규칙을 사용해볼수있습니다.

> 경우에 따라 팀내에서 사용하는 플러그인이 아닌 오픈소스로 배포하고 싶거나 npm패키지로 관리하고 싶다면  [관련 아티클](https://kdydesign.github.io/2020/08/28/npm-tutorial/)을 참고해보세요

### 발전시켜보기
Custom 규칙 만들기는 여기서 마무리하지만, 추가적으로 해볼만한것들이 아직 남아있습니다.

- 옵션을 허용하여 특정 파일의 경우 이름을 자유롭게 설정하거나 특정 이름을 가지도록 설정하기
- typescript에서 지원이 가능하도록 테스트 코드를 보강하기

관심이 있으시다면 한번 시도해보는것도 좋을것 같습니다.

> 전체 예제코드는 [github 저장소](https://github.com/developer-bandi/eslint-custom-plugin-example/tree/main)에서 보실수 있습니다.

## 마치며

이번 아티클에서는 eslint custom plugin을 직접 만들어 보았습니다. 사실 처음 접했을때는 꽤 어려워보이지만 AST만 이해하면 왠만한 규칙은 다 만들수 있을것 같다는 생각이 들정도로 AST에 대한 이해도가 규칙을 만드는데 있어서 꽤 크게 작용합니다. 특히 AST는 ESLint 뿐만 아니라 babel이나 JSCodeShift와 같은 도구를 사용할때도 도움이 되기 때문에 이번기회에 한번 깊이있게 살펴보면 좋을것 같습니다.
