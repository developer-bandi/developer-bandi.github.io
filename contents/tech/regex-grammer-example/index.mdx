...
여러분은 프로그래밍을 하실때 정규표현식을 자주 사용하시나요? 사실 프로그래밍을 하면서 정규표현식을 사용할만한 상황이 많지 않다보니 아마 자주 사용하시는 분은 많지 않으실것입니다. 그러다보니 정확하게 익혀서 사용하기 보다는 사례에 맞는 정규표현식을 구글이나 chat gpt에서 검색해 사용하는 경우가 많은것 같습니다.

사실 일반적인 상황에서는 정규표현식 자체가 간단하다보니 테스트 케이스도 많지 않아 정규표현식의 유효성을 증명하기 쉽기에 이러한 방식에 큰 문제가 없지만, 다양한 조건을 고려하는 정규표현식을 작성해야하는경우 정규표현식이 복잡하다보니 생각치도 못한 케이스가 포함되거나 원하는 케이스가 누락되는 상황이 발생 할 수 있습니다. 

따라서 이번 아티클에서는 정규표현식을 정확하게 이해하고 사용할 수 있도록 기본부터 심화 문법까지 살펴보고 실무에서 마주할만한 상황에 이를 적용해보면서 정규표현식을 어떻게 사용할수 있는지 알아봅니다. 

> 정규표현식은 언어나 컴파일러에 따라 문법과 동작이 상이할수 있습니다. 해당 아티클에서는 Javascript와 V8엔진을 기준으로 정규표현식을 설명합니다.

## 정규표현식 문법 이해하기
정규표현식이란 특정한 규칙을 가진 문자열의 집합을 표현하는데 사용하는 형식 언어입니다. 한글, 영어와 같은 자연어나 Java, Javascript와 같은 프로그래밍 언어 처럼 언어로 불립니다. 다만 자연어나 프로그래밍 언어에 비해 표현력이 높지는 않고 할수 있는것은 문자열의 집합을 표현하는것이 가능합니다. 

예를 들어 길이가 2이면서 모두 숫자로 구성되어야한다는 규칙을 가진 문자열을 생각해봅시다. 규칙을 만족하는 모든 문자열을 집합으로 표현하면 `{ "00" , "01" , "02", ... "98" , "99" }` 와 같이 100개의 숫자를 모두 적어야합니다. 하지만 정규표현식으로 작성할경우 간단하게 `\d{2}`와 같이 표기할수 있습니다.

이처럼 정규표현식이란 문자열 집합을 좀더 간단하게 표현하기 위해서 사용하는 언어라고도 볼 수 있겠습니다.


### 자바스크립트 정규표현식
이번 아티클은 자바스크립트를 사용하기 때문에 정규표현식을 자바스크립트에서 사용하기위해 생성, 플래그, 매칭함수에 대한 기본적인 이해가 필요합니다. 따라서 정규표현식 문법을 배우기 앞서 세가지 요소들에 대해서 살펴보겠습니다.

#### 생성
자바스크립트에서 정규표현식을 생성하기 위해 두가지 방법을 사용할 수 있습니다. 바로 리터럴과 RegExp 생성자입니다. 리터럴을 사용하는 경우 스크립트가 로드될때 컴파일되므로 성능상 약간의 이점이 있고, RegExp 생성자를 사용하는 경우 정규식이 실행시점에 컴파일되므로 성능상 약간의 손해가 있지만 동적으로 정규표현식을 생성할때 유리합니다. 
```javascript
const literalRegex = /a/;
const constructorRegex = new RegExp('a');
```
> 아래 예제에서는 보다 직관적인 표기를 위해 리터럴 표기법을 사용합니다.

#### 매칭함수
자바스크립트에서 생성한 정규표현식을 사용하려면 가능한 메서드를 알아야합니다. RexExp의 메서드인 test, exec와 String의 메서드인 match, replace, search, split에서 사용이 가능합니다.

문자열 내부에 패턴과 일치하는 부분이 존재하는지만 알아내려면 패턴 매치결과에 대한 boolean 혹은 인덱스를 반환하는 test나 search 메서드를 사용하고 매치결과에 관한 추가 정보가 필요하면 일치정보가 담긴 배열을 반환하는 exec나 match메서드를 사용하는것이 좋습니다. 만약 정규표현식을 통해 문자 교체를 진행하는 경우 replace와 replaceAll를 사용하는것이 좋습니다.

> 아래 예제에서는 보다 상세한 매칭 결과 표기를 위해 match 함수를 사용합니다.
...
플래그는 d,g,i,m,s,u,y 7종류가 존재하며 이들은 순서에 상관없이 한꺼번에 여러개를 지정할 수도 있습니다. 아래에서는 주로 사용하는 플래그인 g, i에 대해서 살펴보며, 나머지 플래그에 대해서는 [MDN문서](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_expressions#advanced_searching_with_flags)를 참고해보세요
...
#### 모든 문자열 표기하기 
`.` 문자를 사용하면 모든 문자열에 해당한다는 의미를 표현할 수 있습니다. 이때 줄바꿈(`\n`)은 표함하지 않습니다.
...
#### 기본 기호 
수량자에는 가장 기본적으로 사용되는 세가지 기호가 존재합니다. 바로 `*`, `?`, `+` 입니다. `*`는 0번이상 일치하는것, `?` 는 0번또는 1번 일치하는것, `+`는 1번이상 일치하는것입니다.  
...
#### 중괄호 표현법
수량자를 가장 표기하는 가장 일반적인 방법이 바로 중괄호 표기법입니다. 숫자를 하나만 써서 정확히 n번 일치함을 표기하거나, 숫자를 두개써서 n번 이하 m번 이하 일치 하는 식으로 표기할수 있습니다. 또한 숫자를 하나만 쓰면서 	,를 유지할수 있습니다. 이렇게 표기하면 첫번째 숫자를 비운경우 0을 의미하고, 두번째 숫자를 비운경우 무한대를 의미하게됩니다.
...
#### 탐욕적 수량자와 게으른 수량자
앞서 수량자는 그룹이나 메타문자가 N회 반복됨을 나타내기 위한 기호임을 말씀드렸습니다. 이때 수량자는 기본적으로 탐욕적으로 동작하기 때문에 다양한 경우의 패턴이 매칭될때 가능한 긴 패턴을 반환합니다. 만약 게으르게 동작하도록하고 싶다면 수량자 뒤에 `?`를 붙이면 되며 이경우 다양한 경우의 패턴이 매칭될때 가능한 짧은 패턴을 반환합니다.

정의만으로 탐욕적, 게으른 수량자를 이해하는것은 쉽지 않기 때문에 간단한 예제와 함께 탐욕적, 게으른 수량자의 동작원리를 따라가보겠습니다. 예제로 사용할 입력은 'a "witch" and her "broom" is one' 이고 정규표현식은 탐욕적인경우 `/".+"/g`이며 게으른 경우 `/".+?"/g`입니다. 먼저 탐욕적인 경우 부터 살펴봅시다.
...
위와 같이 탐욕적인 수량자는 가능한 횟수를 많이 반복하고 너무 많이 반복하였다고 생각하면 역추적을 통해 이전 단계로 이동하여 다음 표현식을 매치합니다. 반면 게으른 수량자는 최소한의 횟수로 반복합니다. 이번에는 게으른 수량자의 동작을 살펴봅시다.
...
> 이처럼 탐욕적 수량자와 게으른 수량자는 상황에 따라 전혀 다른 결과를 출력할수 있으므로 다소 난이도가 있는 주제이지만 가급적 정확하게 이해하고 넘어가시는것을 추천드립니다. 탐욕적 수량자와 게으른 수량자에 대한 더 자세한 설명은 [링크](https://javascript.info/regexp-greedy-and-lazy)를 참고해보시면됩니다.


### 그룹화와 캡쳐링
그룹화는 표현식 내부에 동일한 표현식을 두는것으로 소괄호를 사용합니다. 예를들어 `2024-02-11`과 매치되는 정규표현식을 `(\d{4})-(\d{2})-(\d{2})`으로 표기할수 있습니다. 

캡쳐링은 그룹화한 표현식의 결과를 저장하는것입니다. 예를들어 `(\d{4})-(\d{2})-(\d{2})`라는 정규표현식을 사용한경우, 첫번째 괄호, 두번째 괄호, 세번째 괄호의 결과를 따로 저장해 둡니다. 여기서 결과값 뿐만 아니라 정규표현식 자체를 저장하여 현재 정규표현식 내부에서 재사용하는것도 가능합니다.

#### 순서 기반 캡쳐
캡쳐한 그룹을 왼쪽부터 순서대로 저장합니다. 저장한 결과는 정규식 내부 혹은 정규식 결과에서 사용가능합니다. 

정규식 내부에서 사용할 경우 `\1`와 같은 형식(백슬래시 뒤에 순서를 입력)으로 입력해야하고 결과에서 사용할경우 배열의 두번째 요소부터(첫번째 요소는 정규표현식의 결과값) 순서대로 조회하면됩니다.
...
console.log(match[1],match[2],match[3]) // 010 1234 1234 배열의 첫번째는 정규표현식 결과이며 두번재 요소부터 소괄호의 내용이 출력됩니다.
```

#### 이름 지정 캡쳐
순서를 지정하는 방법은 작성하기에 단순하지만 사람이 이해하기 어렵고, 서브 패턴이 추가되는 경우 순서를 파악한뒤 수정해야한다는 단점이 있습니다. 이러한 상황에서는 순서대신 이름을 지정해두고 사용하는 이름 지정 캡쳐를 사용할수 있습니다. 서브 패턴마다 이름을 지정해두고 정규표현식 내부 혹은 결과에서 사용하는것입니다.  

정규식 내부에서 사용할 경우 `\k<name>`와 같은 형식(\k는 고정이고 name만 사용하고자하는 이름으로 변경)으로 입력해야하고 결과에서 사용할경우 groups 프로퍼티에서 저장한 이름 프로퍼티를 조회하면 됩니다.
...
console.log(match.groups.first,match.groups.second,match.groups.third)
```
...
console.log(match[1],match[2],match[3]) // undefined undefined undefined
```
...
console.log(input.match(regex)) // [ '10', index: 14, input: 'This apple is 10$', groups: undefined ] // 달러기호 없이 금액 만을 뽑아 낼 수 있으며 달러가 뒤에 있지 않은 2는 포함되지 않습니다. 
```
...
console.log(input.match(regex)) // [ '2', index: 14, input: 'This apple is 10$', groups: undefined ] // 원화기호가 뒤에 없는 숫자인 2만 출력해낼수 있습니다.
```


#### 후방탐색
후방 탐색은 `(?<=Y)X`와 같은 형식으로 사용하며 Y를 만족하면서 후방(오른쪽)에 X가 있는 경우를 나타냅니다.
...
부정 후방 탐색은 `(?<!Y)X`와 같은 형식으로 사용하며 Y를 만족하면서 후방(오른쪽)에 X가 없는 경우를 나타냅니다.
...
console.log(input.match(regex)) // [ '10', index: 14, input: 'This apple is 10$', groups: undefined ] // 달러기호가 앞에 없는 숫자인 2만 출력해낼수 있습니다.
```

## 구체적인 상황과 함께 살펴보기
정규표현식의 다양한 문법들을 살펴보았습니다. 모든 문법을 전부 살펴본것은 아니지만, 이정도 문법이면 원하는 상황을 구현하는데는 아마 충분할것입니다. 하지만 문법을 배운다고해서 실전에서 적용가능한것은 아닙니다. 프로그래밍 언어와 동일하게 문법을 학습하였으면 문법을 이용하여 실제 결과물을 만들어보는것이 이해에 도움이됩니다.

아래에서는 우리가 일반적으로 실무에서 사용하는 상황들을 가정하여 정규표현식을 실전에서 활용하는 방법에 대해서 살펴봄으로써 정규표현식에 대한 이해를 높여봅시다. 


### commit 메시지 컨벤션 작성하기
대부분의 팀에서는 commit 메시지의 컨벤션이 존재하고 이를 git hooks와 같은 도구를 사용해 커밋메시지를 검사하여 일관된 컨벤션을 유지하고 있을것입니다. 이때 커밋 메시지가 규칙에 맞는지 검사하기 위해서는 정규표현식을 사용하게 됩니다. 이번 예제에서는 일반적인 commit 메시지 룰을 지키는지 검사하는 정규표현식을 한번 만들어봅시다.
...
만들기에 앞서 잠깐 고민해보면 일반적인 방법으로는 만들기가 쉽지 않을것 같다는 생각이 들게됩니다. 왜냐하면 하나의 정규표현식으로 앞선 조건문처럼 원본 입력값에 대해 여러 조건을 적용시키는것이 불가능해보이기 때문입니다. 하지만 앞서 배운 전방탐색을 이용하면 앞서 작성한 조건문처럼 정규표현식을 작성할수 있습니다. 
...
2. `.*[a-z]`는 마지막에 소문자가 존재하는 용어이므로, 여기서는 `'qwer'`이 매치되며 소괄호 패턴이 종료됩니다. 하지만 문자열을 소비하지 않습니다.
3. 다음 정규표현식 `.*`을 매치할때는 전체 문자열 'qwerASDF1234!'에서 시작하므로 모든 문자열이 출력됩니다.
...
## 마치며
이번 아티클을 통해 왠만한 정규표현식을 해석할수 있고, 특정 상황에 대한 정규표현식을 작성할수 있게되었을것입니다. 사실 그렇다고 해서 모든 정규표현식을 직접 작성할 필요는 없다고 생각합니다. chat gpt나 구글의 도움을 받으면서 결과물이 정확한지 검증하는 방식으로 정규표현식을 작성하는것도 좋은 방법이라고 생각하기 때문입니다.


## 참고자료
https://wormwlrm.github.io/2020/07/19/Regular-Expressions-Tutorial.html#%EB%AC%B8%EC%9E%90-%EA%B7%B8%EB%A3%B9character-set-code-classlanguage-textcode-code-classlanguage-textcode
https://javascript.info/regexp-greedy-and-lazy
https://evan-moon.github.io/2020/08/15/regex-example/
https://kasterra.github.io/regex3-capture-and-its-priority/
